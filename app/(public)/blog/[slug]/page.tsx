...
import { formatPostDate, BlogPost } from '@/lib/blog'
...
export async function generateStaticParams() {
  const list: BlogPost[] = (await getBlogsFromAPI()) || [];
...
export async function generateMetadata({
  ...
  const post: BlogPost | null = await getBlogBySlugFromAPI(slug);
...
export default async function BlogPostPage({
  ...
  const { slug } = await params
  const post: BlogPost | null = await getBlogBySlugFromAPI(slug);
  const allPosts: BlogPost[] = (await getBlogsFromAPI()) || [];
  const allCourses: any[] = (await getCoursesFromAPI()) || [];

  if (!post) notFound()

  const relatedCourse = post.relatedCourseSlug
    ? allCourses.find((c: any) => c.slug === post.relatedCourseSlug)
    : undefined;
        ...
        <Container className="max-w-3xl">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-4 text-sm text-white/80">
            <span className="flex items-center gap-2">
              <img
                src={getBackendImageUrl(post.authorPhoto)}
                alt=""
                width={32}
                height={32}
