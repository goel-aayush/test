...
export default async function HomePage() {
  const courseList: Course[] = (await getCoursesFromAPI()) || [];
  const testimonialList = (await getTestimonialsFromAPI()) || [];
  const faqList = (await getFaqsFromAPI('home')) || [];
  const apiSettings = await getSettingsFromAPI();
  const statsList = apiSettings?.stats || [];
      ...
      <section className="bg-muted py-14 sm:py-20">
        <Container className="grid items-center gap-10 lg:grid-cols-2">
          <div className="overflow-hidden rounded-2xl border border-border">
            <SafeImage
              src={getBackendImageUrl('/uploads/general/students-group.png')}
